	...
	<div id="content">
		...
		<form:form method="POST" action="${addAction}" commandName="examForm">
			<table>
				...
				<tr>
					<td><form:label path="cfu">
							<spring:message text="CFU" />
						</form:label></td>
					<td><form:input path="cfu" /></td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="2"><c:if test="${!empty exam.name}">
							<input type="submit" value="<spring:message text="Edit Exam"/>" />
						</c:if> <c:if test="${empty exam.name}">
							<input type="submit" value="<spring:message text="Add Exam"/>" />
						</c:if></td>
				</tr>
			</table>
		</form:form>
